...
type RatingDistribution = Awaited<
  ReturnType<typeof RatingService.getRatingDistribution>
>;
...
function resolveMetadataImageUrl(thumbnailUrl?: string | null) {
  ...
  return /^https?:\/\//i.test(thumbnailUrl)
    ? thumbnailUrl
    : buildAbsoluteUrl(thumbnailUrl);
}
...
const resolveGameDetailBySlug = cache(async function resolveGameDetailBySlug(
  ...
  if (canUseDatabase) {
    try {
      ...
      console.warn(
        'Failed to fetch game from database, falling back to mock data:',
        error
      );
    }
  }
  ...
  if (dbGame) {
    return dbGame.status === 'active'
      ? { game: dbGame, isMockGame: false }
      : null;
  }
...
export async function generateMetadata({
  ...
  const description =
    editorialContent?.metaDescription ?? resolveGameDescription(game, locale);
...
export default async function GamePage({ params }: GamePageProps) {
  ...
  const editorialContent = getGameEditorialContent(game.slug, locale);
  const description =
    editorialContent?.summary ?? resolveGameDescription(game, locale);
  ...
  const resolveLabel = (name?: string | null, nameEn?: string | null) =>
    locale === 'en' ? pickLocalized(nameEn, name) : pickLocalized(name, nameEn);
  ...
  const pageUrl = buildAbsoluteUrl(
    getLocalizedPath(locale, `/games/${game.slug}`)
  );
  const publishedIso = (() => {
    if (!game.publishedAt) return undefined;
    const value =
      game.publishedAt instanceof Date
        ? game.publishedAt
        : new Date(game.publishedAt);
  ...
  const distributionByStars: Record<1 | 2 | 3 | 4 | 5, number> = {
    1:
      ...
      (ratingDistribution as Record<number, number>)[1] ??
      0,
    2:
      ratingDistribution['2'] ??
      (ratingDistribution as Record<number, number>)[2] ??
      0,
    3:
      ratingDistribution['3'] ??
      (ratingDistribution as Record<number, number>)[3] ??
      0,
    4:
      ratingDistribution['4'] ??
      (ratingDistribution as Record<number, number>)[4] ??
      0,
    5:
      ratingDistribution['5'] ??
      (ratingDistribution as Record<number, number>)[5] ??
      0,
  };
  ...
  const distributionEntries = [5, 4, 3, 2, 1].map(stars => {
    ...
    const percentage =
      totalDistributionCount > 0
        ? Math.round((count / totalDistributionCount) * 100)
        : 0;
  ...
  const sourceHost =
    sourceDisclosure.sourceHost ?? sourceDisclosure.embedHost ?? null;
  const sourceLabel =
          ...
          <div className="lg:col-span-2">
            ...
            {catalogueUi.showReviews ? (
              <Card className="mb-8">
                <CardHeader>
                  <CardTitle>
                    {locale === 'zh' ? '玩家评价' : 'Player Reviews'}
                  </CardTitle>
                  <CardDescription>
                    {formatNumber(ratingCount)}{' '}
                    {locale === 'zh' ? '条评价' : 'reviews'}
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="space-y-4">
                    {recentRatings.length > 0 ? (
                      recentRatings.map(rating => {
                        const displayName =
                          locale === 'zh' ? '匿名玩家' : 'Anonymous player';
              ...
              <CardContent className="space-y-4 text-sm">
                ...
                {game.sourceUrl && sourceDisclosure.embedPermissionStatus === 'verified' && (
                  <div className="rounded-lg border border-green-200 bg-green-50/70 p-3 dark:border-green-800 dark:bg-green-950/30">
                    ...
                    >
                      <span>
                        {locale === 'zh' ? '打开来源页面' : 'Open Source Page'}
                      </span>
                      <span>↗</span>
                    </a>
              ...
              >
                <CardHeader>
                  <CardTitle className="text-lg">
                    {locale === 'zh'
                      ? '想试试 3D 贪吃蛇？'
                      : 'Try an original 3D snake challenge'}
                  </CardTitle>
                  <CardDescription>
                    {locale === 'zh'
                          ...
                          <Link
                            ...
                            href={
                              categoryEntries.some(
                                ...
                                ? getLocalizedPath(
                                    locale,
                                    `/games/category/${entry.slug}`
                                  )
                                : getLocalizedPath(
                                    locale,
                                    `/games/tag/${entry.slug}`
                                  )
                            }
