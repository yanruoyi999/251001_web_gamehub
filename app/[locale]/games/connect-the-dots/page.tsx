...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildOriginalExperimentMetadata(getOriginalExperimentPage('connect-the-dots', locale), locale);
}

export default async function ConnectTheDotsPage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const page = getOriginalExperimentPage('connect-the-dots', locale);
  ...
  return (
    <OriginalExperimentPage
      ...
      game={<ConnectTheDots locale={locale} />}
    />
  );
}
