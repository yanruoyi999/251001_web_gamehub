...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildOriginalExperimentMetadata(getOriginalExperimentPage('mahjong-connect', locale), locale);
}

export default async function MahjongConnectPage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const page = getOriginalExperimentPage('mahjong-connect', locale);
  ...
  return (
    <OriginalExperimentPage
      ...
      game={<MahjongConnect locale={locale} />}
    />
  );
}
