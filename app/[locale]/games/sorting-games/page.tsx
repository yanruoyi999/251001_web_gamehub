...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildOriginalExperimentMetadata(getOriginalExperimentPage('sorting-games', locale), locale);
}

export default async function SortingGamesPage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const page = getOriginalExperimentPage('sorting-games', locale);
