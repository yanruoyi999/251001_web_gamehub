...
const copy = {
  en: {
    ...
    gameDescription:
      'A replayable seeded card challenge. Select a face-up card, then choose its foundation or a tableau column.',
    ...
    selected: 'Selected. Choose a foundation or another tableau column.',
  ...
  zh: {
    ...
    gameDescription: '可重复的固定种子牌局。先选择明牌，再选择花色基础堆或另一个 Tableau 列。',
    play: '开始今日牌局',
    replay: '重玩这一天',
    draw: '抽牌',
    drawMode: '抽牌模式',
    ...
    drawThree: '抽 3 张',
    ready: '点击开始，打开这一天的牌局。',
    selected: '已选牌。请选择基础堆或另一个 Tableau 列。',
...
export function DailySolitaire({ locale }: DailySolitaireProps) {
  ...
  const [game, setGame] = useState<DailySolitaireGameState>(() =>
    createDailySolitaireDeal(getDailySolitaireDateKey()),
  );
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(`luma-daily-solitaire:${dateKey}`);
      const saved = raw ? JSON.parse(raw) as { best?: number; streak?: number } : null;
      setBestScore(Number.isFinite(saved?.best) ? saved?.best ?? 0 : 0);
      setStreak(Number.isFinite(saved?.streak) ? saved?.streak ?? 0 : 0);
    } catch {
      setBestScore(0);
      setStreak(0);
    }
  ...
  const resetDeal = (nextDateKey = dateKey, nextStarted = false) => {
    ...
    setElapsed(0);
    startedAtRef.current = nextStarted ? Date.now() : null;
  ...
  const commit = (next: DailySolitaireGameState | null, action: string, properties: Record<string, string | number | boolean> = {}) => {
    ...
    if (complete) {
      const nextScore = Math.max(0, 1000 - next.moves * 5 - next.hints * 20 - elapsed);
      const nextStreak = streak + 1;
      setBestScore((previous) => Math.max(previous, nextScore));
      setStreak(nextStreak);
      try {
        window.localStorage.setItem(
          `luma-daily-solitaire:${dateKey}`,
          JSON.stringify({ best: Math.max(bestScore, nextScore), streak: nextStreak }),
        );
      } catch {
        // The deal remains complete when storage is disabled.
      }
  ...
  const handleTableau = (to: number) => {
    if (!started || !selection) return false;
    const next =
      selection.source === 'waste'
  ...
  const handleDoubleClick = (event: MouseEvent, card: SolitaireCard, pileIndex: number, cardIndex: number) => {
    ...
    if (!started || !card.faceUp || cardIndex !== game.tableau[pileIndex].length - 1) return;
    commit(moveTableauToFoundation(game, pileIndex, card.suit), 'double_click_foundation', { suit: card.suit });
  };

  const handleWaste = () => {
    if (!started || game.waste.length === 0) return;
    ...
    >
      ...
      <div className="space-y-5 p-4 sm:p-6">
        ...
        <div className="flex flex-wrap items-center gap-2" role="toolbar" aria-label={locale === 'zh' ? '牌局工具' : 'Deal tools'}>
          <button type="button" onClick={handleDraw} disabled={!started} className="inline-flex min-h-10 items-center gap-2 rounded-md border border-slate-700 px-3 text-sm font-semibold text-slate-200 transition hover:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300">
        ...
        <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-1">
            <div>
              ...
              <button type="button" onClick={handleDraw} disabled={!started} aria-label={`${text.draw} ${drawCount}`} className="flex aspect-[4/3] min-h-24 w-full items-center justify-center rounded-xl border-2 border-dashed border-slate-700 bg-slate-900 text-sm font-bold text-slate-400 transition hover:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300">
              ...
              {SOLITAIRE_SUITS.map((suit) => {
                ...
                const canReceive = selection
                  ? selection.source === 'waste'
                    ...
                    : Boolean(game.tableau[selection.pileIndex]?.at(-1) && game.tableau[selection.pileIndex]?.at(-1)?.suit === suit && canMoveToFoundation(game.tableau[selection.pileIndex].at(-1) as SolitaireCard, foundation))
                  : false;
                return (
            ...
            {game.tableau.map((pile, pileIndex) => (
              <div key={pileIndex} className="min-h-40 rounded-xl border border-slate-800 bg-slate-900/50 p-2" onClick={() => handleTableau(pileIndex)}>
                ...
                <div className="flex min-h-28 flex-col items-stretch gap-1">
                  ...
                  {pile.length === 0 ? <span className="flex min-h-12 items-center justify-center rounded-md border border-dashed border-slate-700 text-xs text-slate-600">{text.empty}</span> : null}
                </div>
              </div>
            ))}
          </div>
