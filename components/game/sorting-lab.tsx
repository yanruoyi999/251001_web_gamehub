...
export function SortingLab({ locale }: SortingLabProps) {
  ...
  return (
    <section data-sorting-lab data-interactive-ready={interactiveReady} aria-busy={!interactiveReady} inert={interactiveReady ? undefined : true} aria-labelledby="sorting-lab-title" className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 text-white shadow-xl">
      <header className="flex flex-col gap-5 border-b border-slate-800 p-5 sm:p-7 md:flex-row md:items-end md:justify-between">
        ...
        <button type="button" onClick={startMode} disabled={!interactiveReady} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-amber-300 px-4 py-3 font-bold text-slate-950 transition hover:bg-amber-200 disabled:cursor-wait disabled:opacity-60 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-100">
