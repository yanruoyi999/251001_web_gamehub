...
- CI 复核：首个 `main` run 在 `actions/setup-node@v5` 的自动 pnpm 缓存阶段失败，因为 pnpm 尚未安装；业务测试尚未启动。三个 workflow 已统一为先执行 Node 24 runtime 的 `pnpm/action-setup@v6`、再执行带 pnpm cache 的 `setup-node@v5`；修复后必须以新的完整 CI run 作为发布门禁，不把首次失败覆盖或忽略。
