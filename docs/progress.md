...
- 验证结果：目标测试 3 files / 10 tests、完整 Vitest 61 files / 198 tests、CI 模式 Playwright 13/13、内链检查、type-check、lint（0 errors / 98 个既有 no-console warnings）、`pnpm audit:prod`（No known vulnerabilities found）、Next.js 15.5.21 production build（139/139 静态页）和 `git diff --check` 通过。页面质量 255 rows / 91 indexable / 0 indexable under-80。CI 首次失败仅因旧 E2E 仍断言 OvO 推荐卡片；测试契约已改为验证 manual-review OvO 不出现且 Tunnel Rush 卡片仍可打开详情。
...
### T-160 留存入口可发现性修复（2026-08-13）

- 生产复现：首页“今日推荐”位于四张精选卡后约 992px；游戏详情的收藏只是标题右侧小型灰色按钮，推荐位于右栏游戏信息和提示之后。功能存在但用户无法快速发现，这是布局缺陷，不是缓存、路由或部署失败。
...
- CI 复核：首个 `main` run 在 `actions/setup-node@v5` 的自动 pnpm 缓存阶段失败，因为 pnpm 尚未安装；业务测试尚未启动。`ci.yml` 与手动 production smoke 已统一为先执行 `pnpm/action-setup@v4`、再执行带 pnpm cache 的 `setup-node@v5`，与已成功的 IndexNow workflow 顺序一致；修复后必须以新的完整 CI run 作为发布门禁，不把首次失败覆盖或忽略。
