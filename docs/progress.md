...
- 提交部署：commit `b02c4f8 add fifth batch core game guides` 已推送 `origin/main`；因 GitHub 自动部署未立即切主域，已从干净临时 worktree 手动执行 `vercel deploy --prod --yes`；Vercel production deployment `dpl_9wg6eLFEa7RzRL8QUiWUR4B92t15` Ready，并挂载 `https://www.lumagamehub.com`。
