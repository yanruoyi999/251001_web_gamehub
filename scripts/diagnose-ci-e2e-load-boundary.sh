...
validate_evidence() {
  ...
  for path in "$logs/server.stdout.log" "$logs/server.stderr.log" "$logs/health.err"; do [[ -f "$path" ]] || { echo "missing retained receipt: $path" >&2; failed=1; }; done
  [[ ! -s "$logs/health.err" ]] || { echo 'successful health stderr is not empty' >&2; failed=1; }
  for route in /api/health /en /en/games /en/guides/google-snake-mods; do grep -Fq "$(printf '\t')$route$(printf '\t')" "$logs/readiness-probes.tsv" 2>/dev/null || { echo "missing readiness probe: $route" >&2; failed=1; }; done
  ...
  if ! jq -e --argjson expected "$expected" 'type=="array" and length==$expected' "$playwright/cases.json" >/dev/null 2>&1; then echo 'actual case selection evidence is incomplete' >&2; failed=1; fi
  for index in $(seq 1 "$expected"); do
    ...
    find "$playwright/case-$index/results" -type f -name '*.zip' -size +0c -print -quit 2>/dev/null | grep -q . || { echo "missing trace evidence: $index" >&2; failed=1; }
    find "$playwright/case-$index/results" -type f -name '*.webm' -size +0c -print -quit 2>/dev/null | grep -q . || { echo "missing video evidence: $index" >&2; failed=1; }
    find "$playwright/case-$index/results" -type f -name '*.png' -size +0c -print -quit 2>/dev/null | grep -q . || { echo "missing screenshot evidence: $index" >&2; failed=1; }
    grep -Eq 'pw:api|/_next/image|RSC' "$logs/case-$index.log" 2>/dev/null || { echo "missing request trace evidence: $index" >&2; failed=1; }
  done
  return "$failed"
}
