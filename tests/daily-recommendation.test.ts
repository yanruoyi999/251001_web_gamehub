...
describe('daily recommendation', () => {
  ...
  it('returns three unique recommendations for the visible homepage module', () => {
    ...
    expect(recommendations.every((entry) => Number.isInteger(entry.gameId))).toBe(true);
  });

  it('excludes the current game from a multi-card recommendation set', () => {
    const recommendations = getDailyRecommendations('2026-08-13', 'google-snake', 3);

    expect(recommendations.map((entry) => entry.slug)).not.toContain('google-snake');
  });
